...
mono_MHI_all <- full_join(mono_cs, mono_ci, by = c("plate_ID", "Well", "plate", "sampleName")) %>%
...
# n normalization  
nDNA_av = mean(mono_MHI_all$nNORM, na.rm = T)
...
MiSBIE_mono_mhi_protein <- readxl:: read_xlsx ("interim_data/protein.xlsx") %>%
  filter(plate == "mono") %>%
...
mono_MHI_all <- mono_MHI_all %>%
  full_join(MiSBIE_mono_mhi_protein) %>%
...
write_xlsx(mono_MHI_all, "final_data/manual/mono_MHI_all_manual.xlsx")
