...
PBMC_F.1_MHI_all <- full_join(PBMC_F.1_cs, PBMC_F.1_ci, by = c("plate_ID", "Well", "plate", "sampleName")) %>%
  # dplyr::select("plate_ID", "Well", "plate", "sampleName", "cs_cell", "ci_cell") %>%
  full_join(PBMC_F.1_sdh) %>%
  # dplyr::select(-"sdh") %>%
  full_join(PBMC_F.1_cox) %>%
  # dplyr::select(-"cox") %>%
  full_join (PBMC_F.1_qpcr) %>%
...
PBMC_F.1_MHI_all <- PBMC_F.1_MHI_all %>%
  mutate(cs = case_when(
...
# n normalization  
nDNA_av = mean(PBMC_F.1_MHI_all$nNORM, na.rm = T)
...
MiSBIE_PBMC_F.1_mhi_cellcount <- readxl:: read_xlsx ("raw_data/MiSBIE_mhi_cellcount.xlsx") %>%
  filter(plate == "PBMC_F.1") %>%
  dplyr::select(!cell_type) %>%
  transform(cell_count = as.numeric(cell_count)) %>%
  mutate(cell_adj = 5/cell_count)

PBMC_F.1_MHI_all <- PBMC_F.1_MHI_all %>%
  full_join(MiSBIE_PBMC_F.1_mhi_cellcount, by = c("sampleName", "plate")) %>%
...
MiSBIE_PBMC_F.1_mhi_protein <- readxl:: read_xlsx ("interim_data/protein.xlsx") %>%
  filter(plate == "PBMC_F.1") %>%
...
PBMC_F.1_MHI_all <- PBMC_F.1_MHI_all %>%
  full_join(MiSBIE_PBMC_F.1_mhi_protein) %>%
...
write_xlsx(PBMC_F.1_MHI_all, "final_data/manual/PBMC_F.1_MHI_all_manual.xlsx")
