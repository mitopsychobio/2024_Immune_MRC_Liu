...
neu_ci <- readxl::read_xlsx("interim_data/manual/neu_ci.xlsx", sheet = 1) %>%
  ...
  unite(plate_ID, plate, sampleName, sep = "_", remove = F) %>%
  filter(!sampleName %in% c("EMPTY",NA))
neu_sdh <- readxl::read_xlsx("interim_data/manual/neu_sdh.xlsx", sheet = 1) %>%
...
neu_MHI_all <- full_join(neu_cs, neu_ci, by = c("plate_ID", "Well", "plate", "sampleName")) %>%
...
# n normalization  
nDNA_av = mean(neu_MHI_all$nNORM, na.rm = T)
...
MiSBIE_neu_mhi_cellcount <- readxl:: read_xlsx ("raw_data/MiSBIE_mhi_cellcount.xlsx") %>%
  filter(plate == "neu") %>%
...
neu_MHI_all <- neu_MHI_all %>%
  full_join(MiSBIE_neu_mhi_cellcount, by = c("sampleName", "plate")) %>%
...
MiSBIE_neu_mhi_protein <- readxl:: read_xlsx ("interim_data/protein.xlsx") %>%
  filter(plate == "neu") %>%
...
neu_MHI_all <- neu_MHI_all %>%
  full_join(MiSBIE_neu_mhi_protein) %>%
...
write_xlsx(neu_MHI_all, "final_data/manual/neu_MHI_all_manual.xlsx")
