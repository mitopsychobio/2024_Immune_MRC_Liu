...
MiSBIE_neu <- readxl::read_xlsx("plate_maps/MiSBIE_neu.xlsx", sheet = 1)%>%
  rename(row_96 = Row, column_96 = Column, well_96 = Well)


#read files----

MiSBIE_neu_nd1_b2m_A <- readxl::read_xls("raw_data/qPCR_data/neutrophils/MiSBIE_MHI_neutrophils_A_ND1_B2M.xls", sheet = 3)[44:812,] %>%
  row_to_names(row_number = 1) %>%
  select(`Well Position`,`Target Name`, CT) 
MiSBIE_neu_nd1_b2m_B <- readxl::read_xls("raw_data/qPCR_data/neutrophils/MiSBIE_MHI_neutrophils_B_ND1_B2M.xls", sheet = 3)[44:812,] %>%
  row_to_names(row_number = 1) %>%
  select(`Well Position`,`Target Name`, CT) 

MiSBIE_neu_cox1_rnasep_A <- readxl::read_xls("raw_data/qPCR_data/neutrophils/MiSBIE_MHI_neutrophils_A_COXI_RNaseP.xls", sheet = 3)[44:812,] %>%
  row_to_names(row_number = 1) %>%
  select(`Well Position`,`Target Name`, CT)
MiSBIE_neu_cox1_rnasep_B <- readxl::read_xls("raw_data/qPCR_data/neutrophils/MiSBIE_MHI_neutrophils_B_COXI_RNaseP.xls", sheet = 3)[44:812,] %>%
  row_to_names(row_number = 1) %>%
  select(`Well Position`,`Target Name`, CT) 
...
#nd1_A
MiSBIE_neu_nd1_A <- MiSBIE_neu_nd1_b2m_A %>%
  filter(`Target Name` == "ND1") %>%
  rename(Well = `Well Position`) %>%
  full_join(qPCR_platemap, MiSBIE_neu_nd1_A, by = "Well") %>%
...
MiSBIE_neu_b2m_A <- MiSBIE_neu_nd1_b2m_A %>%
  filter(`Target Name` == "B2M") %>%
  rename(Well = `Well Position`) %>%
  full_join(qPCR_platemap, MiSBIE_neu_b2m_A, by = "Well")%>%
  select(sample_no, replicate, Well, CT) %>%
  pivot_wider(names_from = replicate, values_from = c(Well, CT)) %>%
  na.omit() %>% select(-c("Well_NA", "CT_NA")) %>%
  arrange(by_group = sample_no) %>%
...
MiSBIE_neu_nd1_B <- MiSBIE_neu_nd1_b2m_B %>%
  filter(`Target Name` == "ND1") %>%
  rename(Well = `Well Position`) %>%
  full_join(qPCR_platemap, MiSBIE_neu_nd1_B, by = "Well")%>%
  select(sample_no, replicate, Well, CT) %>%
  pivot_wider(names_from = replicate, values_from = c(Well, CT)) %>%
  na.omit() %>% select(-c("Well_NA", "CT_NA")) %>%
  arrange(by_group = sample_no) %>%
...
MiSBIE_neu_b2m_B <- MiSBIE_neu_nd1_b2m_B %>%
  filter(`Target Name` == "B2M") %>%
  rename(Well = `Well Position`) %>%
  full_join(qPCR_platemap, MiSBIE_neu_b2m_B, by = "Well")%>%
  select(sample_no, replicate, Well, CT) %>%
  pivot_wider(names_from = replicate, values_from = c(Well, CT)) %>%
  na.omit() %>% select(-c("Well_NA", "CT_NA")) %>%
  arrange(by_group = sample_no) %>%
...
MiSBIE_neu_nd1_A_ct <- MiSBIE_neu_nd1_A %>%
  select(sampleName, sample_no, nd1_ct_cleaned_mean) %>%
  rename(nd1_A_ct = nd1_ct_cleaned_mean) %>%
  na.omit()
MiSBIE_neu_nd1_B_ct <- MiSBIE_neu_nd1_B %>%
  select(sampleName, sample_no, nd1_ct_cleaned_mean) %>%
  rename(nd1_B_ct = nd1_ct_cleaned_mean) %>%
  na.omit()
MiSBIE_neu_b2m_A_ct <- MiSBIE_neu_b2m_A %>%
  select(sampleName, sample_no, b2m_ct_cleaned_mean) %>%
  rename(b2m_A_ct = b2m_ct_cleaned_mean) %>%
  na.omit()
MiSBIE_neu_b2m_B_ct <- MiSBIE_neu_b2m_B %>%
  select(sampleName, sample_no, b2m_ct_cleaned_mean) %>%
  rename(b2m_B_ct = b2m_ct_cleaned_mean) %>%
  na.omit()

MiSBIE_neu_nd1_b2m <- full_join(MiSBIE_neu_nd1_A_ct, MiSBIE_neu_nd1_B_ct, by = c("sampleName", "sample_no")) %>%
  full_join(MiSBIE_neu_b2m_A_ct, by = c("sampleName", "sample_no"))%>%
  full_join(MiSBIE_neu_b2m_B_ct, by = c("sampleName", "sample_no")) %>%
  arrange(by_group = sample_no) %>%
...
#cox1_A
MiSBIE_neu_cox1_A <- MiSBIE_neu_cox1_rnasep_A %>%
  filter(`Target Name` == "COXI") %>%
  rename(Well = `Well Position`) %>%
  full_join(qPCR_platemap, MiSBIE_neu_cox1_A, by = "Well")%>%
  select(sample_no, replicate, Well, CT) %>%
  pivot_wider(names_from = replicate, values_from = c(Well, CT)) %>%
  na.omit() %>% select(-c("Well_NA", "CT_NA")) %>%
  arrange(by_group = sample_no) %>%
...
MiSBIE_neu_rnasep_A <- MiSBIE_neu_cox1_rnasep_A %>%
  filter(`Target Name` == "RNaseP") %>%
  rename(Well = `Well Position`) %>%
  full_join(qPCR_platemap, MiSBIE_neu_rnasep_A, by = "Well")%>%
  select(sample_no, replicate, Well, CT) %>%
  pivot_wider(names_from = replicate, values_from = c(Well, CT)) %>%
  na.omit() %>% select(-c("Well_NA", "CT_NA")) %>%
  arrange(by_group = sample_no) %>%
...
MiSBIE_neu_cox1_B <- MiSBIE_neu_cox1_rnasep_B %>%
  filter(`Target Name` == "COXI") %>%
  rename(Well = `Well Position`) %>%
  full_join(qPCR_platemap, MiSBIE_neu_cox1_B, by = "Well")%>%
  select(sample_no, replicate, Well, CT) %>%
  pivot_wider(names_from = replicate, values_from = c(Well, CT)) %>%
  na.omit() %>% select(-c("Well_NA", "CT_NA")) %>%
  arrange(by_group = sample_no) %>%
...
MiSBIE_neu_rnasep_B <- MiSBIE_neu_cox1_rnasep_B %>%
  filter(`Target Name` == "RNaseP") %>%
  rename(Well = `Well Position`) %>%
  full_join(qPCR_platemap, MiSBIE_neu_rnasep_B, by = "Well")%>%
  select(sample_no, replicate, Well, CT) %>%
  pivot_wider(names_from = replicate, values_from = c(Well, CT)) %>%
  na.omit() %>% select(-c("Well_NA", "CT_NA")) %>%
  arrange(by_group = sample_no) %>%
...
#average the duplicate plates----
MiSBIE_neu_cox1_A_ct <- MiSBIE_neu_cox1_A %>%
  select(sampleName, sample_no, cox1_ct_cleaned_mean) %>%
  rename(cox1_A_ct = cox1_ct_cleaned_mean) %>%
  na.omit()
MiSBIE_neu_cox1_B_ct <- MiSBIE_neu_cox1_B %>%
  select(sampleName, sample_no, cox1_ct_cleaned_mean) %>%
  rename(cox1_B_ct = cox1_ct_cleaned_mean) %>%
  na.omit()
MiSBIE_neu_rnasep_A_ct <- MiSBIE_neu_rnasep_A %>%
  select(sampleName, sample_no, rnasep_ct_cleaned_mean) %>%
  rename(rnasep_A_ct = rnasep_ct_cleaned_mean) %>%
  na.omit()
MiSBIE_neu_rnasep_B_ct <- MiSBIE_neu_rnasep_B %>%
  select(sampleName, sample_no, rnasep_ct_cleaned_mean) %>%
  rename(rnasep_B_ct = rnasep_ct_cleaned_mean) %>%
  na.omit()

MiSBIE_neu_cox1_rnasep <- full_join(MiSBIE_neu_cox1_A_ct, MiSBIE_neu_cox1_B_ct, by = c("sampleName", "sample_no")) %>%
  full_join(MiSBIE_neu_rnasep_A_ct, by = c("sampleName", "sample_no"))%>%
  full_join(MiSBIE_neu_rnasep_B_ct, by = c("sampleName", "sample_no")) %>%
  arrange(by_group = sample_no) %>%
...
MiSBIE_neu_mtDNAcn <- full_join(MiSBIE_neu_nd1_b2m, MiSBIE_neu_cox1_rnasep, by = c("sampleName", "sample_no")) %>%
...
average_deviation = mean(MiSBIE_neu_mtDNAcn$deviation, na.rm = T)

mtDNAcn_cutoff_a <- average_deviation - 0.2
mtDNAcn_cutoff_b <- average_deviation + 0.2

MiSBIE_neu_mtDNAcn <- MiSBIE_neu_mtDNAcn %>%
  mutate (check = ifelse(deviation > mtDNAcn_cutoff_b | deviation < mtDNAcn_cutoff_a, "check", ""))

#address failures for mtDNAcn ---- 
#lin reg using plate 1 only. in MiSBIE_neu samples used both plates together


MiSBIE_neu_mtDNAcn_summ <- MiSBIE_neu_mtDNAcn %>%
...
MiSBIE_neu_nNORM_summ <- MiSBIE_neu_mtDNAcn_summ %>%
...
MiSBIE_neu_qPCR <- full_join(MiSBIE_neu, MiSBIE_neu_mtDNAcn_summ, by = "sampleName") %>%
  full_join(MiSBIE_neu_nNORM_summ, by = "sampleName") %>%
  # select(well_96, sampleName, mtDNAcn_final, nNORM_final) %>%
  rename(Well = well_96, MiSBIE_neu_mtDNAcn_final = mtDNAcn_final, MiSBIE_neu_nNORM_final = nNORM_final) %>%
  transform(plate = case_when(plate == "neutrophil_F.2" ~ plate, TRUE ~ "neutrophil_F.2"))
write_xlsx(MiSBIE_neu_qPCR,"interim_data/qPCR/MiSBIE_neu_qPCR_all.xlsx")
