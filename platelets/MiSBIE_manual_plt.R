...
plt_MHI_all <- full_join(plt_cs, plt_ci, by = c("plate_ID", "Well", "plate", "sampleName")) %>%
...
# n normalization  
nDNA_av = mean(plt_MHI_all$nNORM, na.rm = T)
...
MiSBIE_plt_mhi_cellcount <- readxl:: read_xlsx ("raw_data/MiSBIE_mhi_cellcount.xlsx") %>%
  filter(plate == "plt") %>%
...
plt_MHI_all <- plt_MHI_all %>%
  full_join(MiSBIE_plt_mhi_cellcount, by = c("sampleName", "plate")) %>%
...
MiSBIE_plt_mhi_protein <- readxl:: read_xlsx ("interim_data/protein.xlsx") %>%
  filter(plate == "plt") %>%
...
plt_MHI_all <- plt_MHI_all %>%
  full_join(MiSBIE_plt_mhi_protein) %>%
...
write_xlsx(plt_MHI_all, "final_data/manual/plt_MHI_all_manual.xlsx")
